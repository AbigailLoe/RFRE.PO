...
imputed_analysis = function() {
    ...
    hrf.param2 = hrf.param = hrf.se = hrf.se2 =
        ...
        c.hrf = c.wald= c.summer =
        po.hrf = po.wald= po.summer =
        po.hrf.se  = po.wald.se = po.summer.se =
        c.hrf.se  = c.wald.se = c.summer.se =  list()
    for(l in 1:10){
        print(l)
        ...
        #########  HTREE #########
        trainDf2 = as.data.frame(train.data[,c("pseudoEst", selnHRF)])
        testDF2 = as.data.frame(test.data[, c("pseudoEst", selnHRF)])
        set.seed(16)
        ...
        set.seed(16)
        pred.hrf =  predict_hrf(htree.rf2,x= testDF2,time=test.data$checkIn,
        ...
        hrf.se2[[l]] = t(data.frame(se = varImportDF2$se.2,
                                    row.names = varImportDF2$Predictor))
        c.hrf[[l]] = rcorr.cens(pred.hrf, Surv( test.data$timeToEvent, test.data$delta))[["C Index"]]
        po.hrf[[l]] = mean((pred.hrf - test.data$pseudoEst)^2)
        ...
        for(i in 1:100){
            print(i/100)
            myboot.ind = sample(1:length(pred.hrf), replace = T)
            blankSE =c(blankSE, rcorr.cens(pred.hrf[myboot.ind], 
                                           Surv( test.data$timeToEvent[myboot.ind], 
                                                 test.data$delta[myboot.ind]))[["C Index"]])
            blankPO =c(blankPO, 
                       mean((pred.hrf[myboot.ind] - test.data$pseudoEst[myboot.ind])^2))
        }
        c.hrf.se[[l]] = sd(blankSE)
        po.hrf.se[[l]] = sd(blankPO)
      ...
      rateExacerb+icslama+icslaba+beta_blocker_use"
        ...
        po.wald[[l]] = mean((currPred - test.data$pseudoEst)^2)
        ...
        for(i in 1:100){
            ...
            blankSE = c(blankSE, rcorr.cens(currPred, 
                                            Surv( test.data$timeToEvent[myboot.ind], 
                                                  test.data$delta[myboot.ind]))[["C Index"]])
            blankPO =c(blankPO, 
                       mean((currPred - test.data$pseudoEst[myboot.ind])^2))
        }
        
        c.wald.se[[l]] = sd(blankSE)
        po.wald.se[[l]]=sd(blankPO)
        ...
        po.summer[[l]] = mean((summerPred - test.data$pseudoEst)^2)
        ...
        for(i in 1:100){
            ...
            blankSE = c(blankSE, rcorr.cens(currPred, 
                                            Surv( test.data$timeToEvent[myboot.ind], 
                                                  test.data$delta[myboot.ind]))[["C Index"]])
            blankPO = c(blankPO, mean((currPred - test.data$pseudoEst[myboot.ind])^2))
        }
        c.summer.se[[l]] = sd(blankSE)
        po.summer.se[[l]] = sd(blankPO)
    }
    ...
    c.hrf.combine =(pool_mi(qhat= c.hrf, u = (unlist(c.hrf.se))^2%*%diag(1, nrow=10)))
    ...
    c.results.df = rbind.data.frame(wald.c.value = summary(c.wald.combine),
                                    hrf.imputed.c.value = summary(c.hrf.combine),
                                    summer.c.value = summary(c.summer.combine))
    
    
    
    po.wald.combine =(pool_mi(qhat= po.wald, u = (unlist(po.wald.se))^2%*%diag(1, nrow=10)))
    po.hrf.combine =(pool_mi(qhat= po.hrf, u = (unlist(po.hrf.se))^2%*%diag(1, nrow=10)))
    po.summer.combine =(pool_mi(qhat= po.summer, u = (unlist(po.summer.se))^2%*%diag(1, nrow=10)))
    po.results.df = rbind.data.frame(wald.po = summary(po.wald.combine),
                                     hrf.po = summary(po.hrf.combine),
                                     summer.po = summary(po.summer.combine))
    
    
    
    write.csv(total.imputed.df, 
              "imputationResults.csv")
    
    
    
    total.imputed.df %>%
        filter(model.type == "wald") %>% 
        mutate(lower = exp(lower),
               upper = exp(upper),
               e.beta = exp(results)) %>% 
        dplyr::select(var.name, e.beta, lower, upper, p) %>% 
        xtable( digits = 2)
